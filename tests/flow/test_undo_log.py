...
class testUndoLog():
    ...
    def disabled_test16_undo_label_set(self):
        ...
        result = self.graph.query("CALL db.labels")
        self.env.assertEqual(result.result_set, [["L1"]])
    ...
    def test16_undo_label_set(self):
        ...
        # check index is ok
        self.env.assertEqual(len(labels), 2)
        self.env.assertContains("L1", labels)
        self.env.assertContains("L2", labels)
        ...
        # check index is ok
        self.env.assertEqual(len(relationships), 1)
        self.env.assertContains("Z", relationships)
